...
if __name__ == '__main__':
    ...
    for i in range(0, num_of_sentences):
        ...
        if be_sentence:
            ...
            _, latter_verb_dep_index = simplifier.query_type(broken_depend_list[2 * i + 1], 'dep', 'dobj')
            latter_verb_word = broken_depend_list[2 * i + 1][latter_verb_dep_index]['governorGloss']
            latter_verb_index = broken_depend_list[2 * i + 1][latter_verb_dep_index]['governor'] - 1
            ...
            latter_subj_index = broken_depend_list[2 * i + 1][latter_subj_dep_index]['dependent'] - 1
            ...
            if broken_depend_list[2*i+1][pron_dep_index]['dep'] == 'nsubj':
                ...
                pron_role = 'o'
        if not be_sentence:
            can_role = narrative_chain.check(pron_role, verb_word, latter_verb_word)
            # return 1 means pron should be the object one
            if can_role != "NO_DECISION":
                if candidate_list[0] == obj_word or candidate_list[1] == subj_word:
                    if can_role:
                        feature[i][0] = 0
                    else:
                        feature[i][0] = 1
